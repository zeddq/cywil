    ...
    async def health_check(self) -> HealthCheckResult:
        ...
        if not self._initialized:
            return HealthCheckResult(
                status=ServiceStatus.UNHEALTHY, message="Service not initialized"
            )
        ...
        try:
            ...
            if service_result.status == ServiceStatus.UNHEALTHY:
                ...
                service_result.status == ServiceStatus.DEGRADED
            ...
            return HealthCheckResult(
                status=ServiceStatus.UNHEALTHY, message=f"Health check error: {str(e)}"
            )
