        ...
        try:
            if chunk.type == "response.created":
                ...
                return StreamEvent(
                    ...
                    metadata={"created_at": getattr(chunk, 'created_at', None)},
                )
                ...
                if event.item.type == "message":
                    ...
                    content = "".join([getattr(c, 'text', '') for c in message.content if hasattr(c, 'text')])
                    return StreamEvent(
                ...
                return StreamEvent(
                    ...
                    metadata={
                        "usage": (
                            event.response.usage.model_dump() if event.response.usage else None
                        ),
            ...
            return StreamEvent(
                type=StreamEventType.ERROR, metadata={"error": str(e), "chunk_type": chunk.type}
            )
            ...
            if event:
                ...
                if event:
                    ...
                    if event.type == StreamEventType.TOOL_CALL:
                        ...
                        if tool_calls_buffer and event.type in [
...
class ContentAccumulator(StreamProcessor):
    ...
    def process_event(self, event: StreamEvent) -> Optional[StreamEvent]:
        if event.type == StreamEventType.TEXT_DELTA and event.content:
            ...
            event.type in [StreamEventType.TEXT_COMPLETE, StreamEventType.MESSAGE_COMPLETE]
            and event.content
        ):
            self.content = event.content
        return event
