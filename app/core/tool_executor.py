...
class ToolExecutor(ServiceInterface):
    ...
    async def _initialize_impl(self) -> None:
        ...
        for tool in tool_registry.list_tools():
            self._circuit_breakers[tool.name] = CircuitBreaker(
                tool.name, self._default_breaker_config
            )
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        details = {
            ...
            "half_open_circuits": sum(
                1 for cb in self._circuit_breakers.values() if cb.state == CircuitState.HALF_OPEN
            ),
        }
        ...
        details["failure_rate"] = int(failed_calls / total_calls * 100) if total_calls > 0 else int(0)

        # Health is degraded if too many circuits are open
        open_ratio = (
            details["open_circuits"] / details["total_tools"] if details["total_tools"] > 0 else 0
        )
    ...
    async def execute_tool(
        self, name: str, arguments: Dict[str, Any], retry_config: Optional[RetryConfig] = None
    ) -> Any:
        """
        ...
        if not circuit_breaker.can_execute():
            raise ServiceUnavailableError(
                service_name=name,
                message="Tool is currently unavailable (circuit open)",
                details={"tool": name, "state": circuit_breaker.state.value},
            )
        ...
        for attempt in range(retry_config.max_retries + 1):
            try:
                ...
                if attempt < retry_config.max_retries:
                    ...
                    logger.error(
                        f"Tool '{name}' failed after {retry_config.max_retries + 1} attempts"
                    )
        ...
        if tool_name:
            ...
            return {
                ...
                "metrics": {
                    ...
                    "last_failure": (
                        cb.metrics.last_failure.isoformat() if cb.metrics.last_failure else None
                    ),
                    "last_success": (
                        cb.metrics.last_success.isoformat() if cb.metrics.last_success else None
                    ),
                },
            }
        else:
...
def validation_middleware(next_handler, tool_name: str, arguments: Dict[str, Any]):
    ...
    async def handler():
        ...
        if not arguments:
            raise ValidationError(tool_name, {"arguments": "Tool requires arguments"})
        return await next_handler()
