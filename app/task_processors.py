...
class DocumentProcessor:
    ...
    def __init__(self, queue: asyncio.Queue, db_manager: Optional[DatabaseManager] = None):
        self.queue = queue
    ...
    async def _initialize_clients(self):
        ...
        try:
            ...
            self.qdrant_client = QdrantClient(host=config.qdrant.host, port=config.qdrant.port)
    ...
    def _chunk_text(self, text: str) -> List[Dict[str, Any]]:
        ...
        for paragraph in paragraphs:
            ...
            end_char = (
                ...
                if start_char != -1
                else current_position + len(paragraph)
            )
        ...
        if not chunks and text.strip():
            chunks.append(
                {"text": text.strip(), "chunk_num": 0, "start_char": 0, "end_char": len(text)}
            )
        ...
        try:
            if file_path.endswith(".txt"):
                ...
                async with aiofiles.open(file_path, "rb") as f:
                    ...
                    fitz_doc = fitz.open(stream=pdf_data, filetype="pdf")  # type: ignore
                    content = ""
    ...
    async def _process_single_document(self, document_id: str):
        ...
        try:
            ...
            async with self.db_manager.get_session() as db:
                ...
                content = await self._read_document_content(document.file_path) if document.file_path else ""
                ...
                for i, chunk in enumerate(chunks):
                    ...
                    qdrant_id = str(
                        uuid.uuid5(
                            uuid.NAMESPACE_URL,
                            hashlib.sha1(str(document.file_path).encode()).hexdigest(),
                        )
                    )
