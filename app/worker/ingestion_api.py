...
from app.core.auth_middleware import get_current_user, User
...
async def ingest_statutes(
    request: StatuteIngestionRequest,
...
async def run_full_pipeline(
    ...
    try:
        ...
        task = run_full_pipeline.delay(
            request.statute_force_update,
            request.ruling_pdf_directory,
            request.max_ruling_workers
        )
        
        return TaskResponse(
            task_id=task.id,
            status="submitted",
            message="Full pipeline execution submitted"
        )
    except Exception as e:
...
async def run_statute_pipeline(
    ...
    try:
        ...
        return TaskResponse(
            task_id=task.id,
            status="submitted",
            message="Statute pipeline execution submitted"
        )
    except Exception as e:
...
async def run_ruling_pipeline(
    ...
    try:
        ...
        task = run_ruling_pipeline.delay(
            request.pdf_directory,
            request.max_workers
        )
        ...
        return TaskResponse(
            task_id=task.id,
            status="submitted",
            message="Ruling pipeline execution submitted"
        )
    except Exception as e:
