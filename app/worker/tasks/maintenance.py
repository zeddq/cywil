...
def _resolve_maybe_awaitable(value: Any) -> Any:
    """Return result of value, awaiting if it's a coroutine."""
...
def _as_text(value: Any) -> Optional[str]:
    """Convert Redis return value (bytes/str/list/None) to text or None."""
    if value is None:
        return None
    ...
    if isinstance(value, list) and value:
        # Some redis ops with count can return list
        return _as_text(value[0])
    return str(value)


@celery_app.task(name="worker.tasks.maintenance.health_check_all_services", bind=True)
def health_check_all_services(self) -> Dict[str, Any]:
    ...
    async def _process():
        ...
        try:
            ...
            health_report = {"status": "healthy", "services": {}, "errors": [], "warnings": []}
            ...
            try:
                ...
                health_report["services"]["database"] = {"status": "unhealthy", "error": str(e)}
            ...
            try:
                ...
                qdrant_client = QdrantClient(
                    host=config_service.config.qdrant.host, port=config_service.config.qdrant.port
                )
                ...
                health_report["services"]["qdrant"] = {"status": "unhealthy", "error": str(e)}
            ...
            try:
                ...
                health_report["services"]["celery_workers"] = {"status": "unknown", "error": str(e)}
                health_report["warnings"].append(f"Celery workers warning: {e}")
...
@celery_app.task(name="worker.tasks.maintenance.cleanup_expired_results", bind=True)
def cleanup_expired_results(self, days_to_keep: int = 7) -> Dict[str, Any]:
    ...
    try:
        ...
        while True:
            scan_result = _resolve_maybe_awaitable(
                redis_client.scan(cursor, match=pattern, count=100)
            )
            if isinstance(scan_result, tuple) and len(scan_result) == 2:
                cursor = int(scan_result[0])
                keys = (
                    list(scan_result[1])
                    if isinstance(scan_result[1], list)
                    else list(scan_result[1] or [])
                )
            else:
                break
            ...
            for key in keys:
                try:
                    # Get task result
                    result_data = _resolve_maybe_awaitable(redis_client.get(key))
                    text = _as_text(result_data)
                    if text:
                        ...
                        if "date_done" in result:
                            date_done = datetime.fromisoformat(
                                result["date_done"].replace("Z", "+00:00")
                            )
                            if date_done < cutoff_time:
                                ...
                                if isinstance(result_data, bytes):
                                    ...
                                    key_size = len(text.encode("utf-8"))
        ...
        logger.info(
            f"Cleanup completed: {cleanup_stats['keys_deleted']} keys deleted, "
...
@celery_app.task(name="worker.tasks.maintenance.process_dead_letter_queue", bind=True)
def process_dead_letter_queue(self, max_retries: int = 1, requeue: bool = False) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        while True:
            ...
            message = _resolve_maybe_awaitable(redis_client.lpop(dlq_key))
            if not message:
                break
            ...
            try:
                msg_text = _as_text(message)
                msg_data = json.loads(msg_text or "{}")
...
@celery_app.task(name="worker.tasks.maintenance.get_worker_statistics", bind=True)
def get_worker_statistics(self) -> Dict[str, Any]:
    ...
    try:
        ...
        task_states = {"SUCCESS": 0, "FAILURE": 0, "PENDING": 0, "RETRY": 0, "REVOKED": 0}
...
@celery_app.task(name="worker.tasks.maintenance.update_vector_indices", bind=True)
def update_vector_indices(self) -> Dict[str, Any]:
    ...
    try:
        ...
        for collection in collections.collections:
            ...
            # Safely compute vectors_size (guarding None types)
            try:
                params = getattr(collection_info.config, "params", None)
                vectors_cfg = getattr(params, "vectors", None) if params is not None else None
                vectors_size = getattr(vectors_cfg, "size", None) if vectors_cfg is not None else None
            except Exception:
                vectors_size = None
...
@celery_app.task(name="worker.tasks.maintenance.monitor_task_performance", bind=True)
def monitor_task_performance(self, time_window_hours: int = 24) -> Dict[str, Any]:
    """
