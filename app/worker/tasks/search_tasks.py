...
def search_statutes(
    ...
    async def _process():
        ...
        try:
            statute_service = StatuteSearchService(config_service)
            await statute_service.initialize()
            
            # Perform search
            results = await statute_service.search(
                query=query,
                code_type=code_type,
                limit=limit,
            ...
            if statute_service:
                await statute_service.shutdown()
...
def search_rulings(
    ...
    async def _process():
        ...
        try:
            ...
            supreme_court_service = SupremeCourtService(db_manager, config_service)
            await supreme_court_service.initialize()
            ...
            # Perform search
            results = await supreme_court_service.search_rulings(**search_params)
...
@celery_app.task(
    name="worker.tasks.search_tasks.hybrid_search",
    bind=True,
    max_retries=3
)
def hybrid_search(
    self,
    query: str,
    search_types: List[str] = ["statutes", "rulings"],
    ...
    try:
        ...
        if "rulings" in search_types:
            ruling_results = search_rulings.apply_async(
                args=[query, None, limit_per_type]
            ).get(timeout=30)
...
@celery_app.task(
    name="worker.tasks.search_tasks.find_similar_cases",
    bind=True
)
def find_similar_cases(
    ...
    async def _process():
        ...
        try:
            ...
            similar_rulings = await supreme_court_service.search_rulings(
            ...
            return {
                "status": "success",
                "case_description": case_description[:200] + "..." if len(case_description) > 200 else case_description,
...
@celery_app.task(
    name="worker.tasks.search_tasks.extract_legal_references",
    bind=True
)
def extract_legal_references(
    ...
    if validate and references["statutes"]:
        ...
        for ref in references["statutes"]:
            ...
            article_num = int(re.match(r"(\d+)", ref["article"]).group(1))
...
@celery_app.task(
    name="worker.tasks.search_tasks.build_search_index",
    bind=True
)
    ...
    async def _process():
        ...
        try:
            if document_type == "statutes":
                ...
                result = generate_statute_embeddings.apply_async(
                    args=["all", force_rebuild]
                ).get(timeout=300)
                ...
                result = generate_ruling_embeddings.apply_async(
                    args=[None, force_rebuild]
                ).get(timeout=300)
