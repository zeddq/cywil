...
from .statute_tasks import ingest_all_statutes, generate_statute_embeddings
from .ruling_tasks import process_ruling_batch
from .embedding_tasks import generate_ruling_embeddings
...
def run_full_pipeline(
    ...
    if ruling_pdf_directory:
        ruling_task = process_ruling_batch.si(
            pdf_directory=ruling_pdf_directory,
            max_workers=max_ruling_workers
        )
        ...
        return chord(parallel_tasks)(aggregate_pipeline_results.s())
    else:
        # Just run statute pipeline
        return statute_chain.apply_async()
