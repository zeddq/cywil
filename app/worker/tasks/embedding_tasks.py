...
from .sn import process_sn_ruling_file
...
def generate_statute_embeddings(code_type: str, force_regenerate: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "completed", "code_type": code_type, "statistics": stats}

    except Exception as e:
        logger.error(f"Error generating statute embeddings: {e}", exc_info=True)
        return {"status": "error", "error": str(e), "code_type": code_type}
...
def generate_ruling_embeddings(jsonl_path: str) -> Dict[str, Any]:
    ...
    async def _process():
        ...
        try:
            ...
            return {"status": "error", "error": str(e), "jsonl_path": jsonl_path}
        finally:
            if db_manager:
...
def batch_generate_embeddings(
    texts: List[str], metadata: Optional[List[Dict[str, Any]]] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"status": "error", "error": str(e), "text_count": len(texts)}
    ...
    async def _process():
        try:
            ...
            if collection_name:
                ...
                return {"status": "completed", "collections": stats}
