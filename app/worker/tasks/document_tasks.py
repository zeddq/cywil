...
def generate_legal_document(
    self, document_type: str, context: Dict[str, Any], user_id: str
) -> Dict[str, Any]:
    """
...
@celery_app.task(name="worker.tasks.document_tasks.validate_document", bind=True, max_retries=2)
def validate_document(
    self, document_content: str, document_type: str, check_citations: bool = True
) -> Dict[str, Any]:
    """
...
@celery_app.task(name="worker.tasks.document_tasks.extract_document_metadata", bind=True)
def extract_document_metadata(
    self, document_content: str, document_type: Optional[str] = None
) -> Dict[str, Any]:
    """
...
@celery_app.task(name="worker.tasks.document_tasks.convert_document_format", bind=True)
def convert_document_format(
    self, document_content: str, source_format: str, target_format: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if source_format == "text" and target_format == "markdown":
            ...
            converted_content = re.sub(
                r"^# (.+)$", r"<h1>\1</h1>", converted_content, flags=re.MULTILINE
            )
            converted_content = re.sub(
                r"^## (.+)$", r"<h2>\1</h2>", converted_content, flags=re.MULTILINE
            )
