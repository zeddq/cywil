        ...
        try:
            ...
            self.client.create_collection(
                ...
                optimizers_config=OptimizersConfigDiff(
                    indexing_threshold=20000, memmap_threshold=50000
                ),
            )
...
def process_and_embed_statutes(
    ...
    embedder = PolishLegalEmbedder(qdrant_host=qdrant_host, qdrant_port=qdrant_port)
...
def create_hybrid_search_index(
    ...
    try:
        client.create_payload_index(
            collection_name=collection_name, field_name="text", field_schema=TextIndexParams(type=qmodels.TextIndexType.text)
        )
...
if __name__ == "__main__":
    # Process KC chunks
    kc_stats = process_and_embed_statutes("data/chunks/KC_chunks.json", collection_name="statutes")
    print(f"KC embedding stats: {kc_stats}")

    # Process KPC chunks
    kpc_stats = process_and_embed_statutes(
        "data/chunks/KPC_chunks.json", collection_name="statutes"
    )
    ...
    # Test search
    results = search_statutes("odpowiedzialność kontraktowa", code_filter="KC", top_k=3)
