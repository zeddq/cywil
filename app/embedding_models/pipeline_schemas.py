...
class RawDocument(BaseModel):
    ...
    id: str = Field(..., pattern=r"^[A-Z0-9-]+$", description="Unique document identifier")
...
class EmbeddedChunk(ProcessedChunk):
    ...
    def validate_embedding_normalized(cls, v):
        """Ensure embedding vector is properly normalized and expected length range."""
        if not (384 <= len(v) <= 1536):
            raise ValueError(f'Embedding length out of bounds: {len(v)}')
