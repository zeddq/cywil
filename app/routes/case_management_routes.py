...
from ..services.case_management_service import CaseManagementService, get_case_management_service
import logging
...
def require_active_user(request: Request):
    if request.state.user and request.state.user.is_active:
        return request.state.user
    else:
        raise HTTPException(
...
async def create_document(
    ...
    if not new_doc:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Case not found for this user")
    return new_doc
