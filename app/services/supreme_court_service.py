...
from sqlalchemy import select, String, cast as sa_cast
...
class SupremeCourtService(ServiceInterface):
    ...
    def __init__(self, db_manager: DatabaseManager, config_service: ConfigService, llm_manager: LLMManager):
    ...
    async def _initialize_impl(self) -> None:
        ...
        print(
            f"self._config.qdrant.api_key.get_secret_value() {self._config.qdrant.api_key.get_secret_value() if self._config.qdrant.api_key else None}"
        )
        ...
        self._qdrant_client = AsyncQdrantClient(
            ...
            api_key=(
                self._config.qdrant.api_key.get_secret_value()
        ...
        if self._config.qdrant.collection_rulings not in collection_names:
            logger.warning(
                f"Collection '{self._config.qdrant.collection_rulings}' not found in Qdrant"
            )
    ...
    @tool_registry.register(
        ...
        parameters=[
            ...
            ToolParameter(
                "filters", "object", "Optional filters (date_from, date_to, section)", False
            ),
        ],
        returns="List of relevant rulings with metadata and excerpts",
    )
    async def search_sn_rulings(
                ...
                results.append(
                    RulingPayload(
                            ...
                            paragraphs=RulingParagraphPayload(
                                    ...
                                    para_no=point.payload.get("para_no", 0),
    ...
    async def _semantic_search(
        ...
        if filters:
            ...
            if "date_to" in filters:
                must_conditions.append(
                    FieldCondition(key="date", range=Range(lte=filters["date_to"]))
                )
    ...
    @tool_registry.register(
        ...
        parameters=[
            ToolParameter(
                "docket",
                "string",
                "The docket number of the Supreme Court ruling (e.g., 'III CZP 2/18')",
                True,
            ),
        ],
    ...
    @tool_registry.register(
        ...
        parameters=[ToolParameter("rulings", "array", "List of rulings to summarize")],
    ...
    async def summarize_sn_rulings(self, rulings: List[RulingPayload]) -> str:
        ...
        prompt_template = """Jako ekspert prawa cywilnego, przeanalizuj poniższe orzeczenia Sądu Najwyższego:
...
Podsumowanie:"""
        ...
        response = await self._llm_manager.generate_completion(prompt_template.format(rulings=rulings_text))
        return response or "Nie można wygenerować podsumowania orzeczeń."

    async def get_ruling_paragraphs(
        self, docket: str, section: Optional[str] = None
    ) -> List[Dict[str, Any]]:
...
Sygnatura: {ruling.meta.get('docket', 'N/A') if ruling.meta else 'N/A'}
Data: {ruling.meta.get('date', 'N/A') if ruling.meta else 'N/A'}
