...
class DocumentGenerationService(ServiceInterface):
    ...
    def __init__(
        ...
        statute_search_service: StatuteSearchServiceDep,
        sn_rulings_service: SupremeCourtServiceDep,
    ):
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        ...
        try:
            ...
            async with self._db_manager.get_session() as session:
                result = await session.execute(select(FormTemplate).limit(1))
                _ = result.scalar()

            return HealthCheckResult(
                status=ServiceStatus.HEALTHY, message="Document generation service is healthy"
            )
    ...
    @tool_registry.register(
        ...
        category=ToolCategory.DOCUMENT,
        parameters=[ToolParameter("template_type", "string", "Type of template to find")],
    ...
    async def find_template(self, template_type: str) -> Optional[Dict[str, Any]]:
        ...
        async with self._db_manager.get_session() as session:
            template = await session.get(FormTemplate, template_type)
    ...
    @tool_registry.register(
        ...
        parameters=[
            ToolParameter(
                "doc_type", "string", "Type of document - use list_available_templates first"
            ),
    ...
    async def draft_document(
        self, doc_type: str, facts: Dict[str, Any], goals: List[str]
    ) -> Dict[str, Any]:
    ...
    async def _enhance_document_with_ai(
        self, template: str, rulings: Dict[str, Any], doc_type: str
    ) -> str:
...
"""
    ...
    @tool_registry.register(
        ...
        parameters=[
            ...
            ToolParameter(
                "analysis_type",
                "string",
                "Type of analysis requested ('summary', 'key_clauses', 'risk_assessment')",
                False,
                "summary",
                enum=["summary", "key_clauses", "risk_assessment"],
            ),
        ],
        returns="Detailed analysis of the contract based on the requested type",
    )
    async def analyze_contract(
        self, contract_content: str, analysis_type: str = "summary"
    ) -> Dict[str, Any]:
        if not self._initialized:
            raise RuntimeError("Service not initialized")
        ...
        prompt = prompt_template.format(
            contract_content=contract_content, analysis_type=analysis_type
        )
    ...
    async def validate_against_statute(self, draft: str, citations: List[str]) -> Dict[str, Any]:
        ...
        for article, code in found_citations + parsed_citations:
            search_results = await self._statute_search.search_statute(
                f"art. {article} {code}", top_k=1, code=code
            )
            ...
            if not search_results:
                ...
                is_valid = await self._validate_citation_context(
                    draft, search_results[0], article_text
                )
    ...
    async def _validate_citation_context(
        self, draft: str, citation_info: Dict[str, Any], article_text: str
    ) -> bool:
...
Czy cytat jest prawidłowy? Odpowiedz TAK lub NIE i podaj krótkie uzasadnienie."""
        ...
        return "TAK" in response.output_text
...
DocumentGenerationServiceDep = Annotated[
    DocumentGenerationService, Depends(get_document_generation_service)
]
