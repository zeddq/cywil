...
class CaseManagementService(ServiceInterface):
    ...
    async def _health_check_impl(self) -> HealthCheckResult:
        return HealthCheckResult(
            status=ServiceStatus.HEALTHY, message="CaseManagementService is healthy"
        )
    ...
    async def create_case(self, case_in: Case, user: User) -> Case:
        return await self.case_repository.create_case(case_in=case_in, user_id=UUID(user.id))

    async def list_cases(self, user: User, status: Optional[str] = None) -> List[Case]:
        return await self.case_repository.list_cases(user_id=UUID(user.id), status=status)

    async def get_case(self, case_id: UUID, user: User) -> Optional[Case]:
        return await self.case_repository.get_case(case_id=case_id, user_id=UUID(user.id))
    ...
    async def create_document(self, doc_in: Document, user: User) -> Optional[Document]:
        ...
        case = await self.get_case(case_id=UUID(str(doc_in.case_id)), user=user)
        if not case:
            return None
    ...
    async def list_documents(self, user: User, case_id: Optional[UUID] = None) -> List[Document]:
        return await self.case_repository.list_documents(user_id=UUID(str(user.id)), case_id=case_id)

    async def upload_document(
        self, case_id: UUID, file: UploadFile, user: User
    ) -> Optional[Document]:
        case = await self.get_case(case_id=case_id, user=user)
        if not case:
            return None

        upload_dir_path = self._config.storage.get_path(self._config.storage.upload_dir)
        original_filename = file.filename or ""
        name, ext = os.path.splitext(original_filename)
        safe_filename = original_filename if original_filename else f"upload_{datetime.now().timestamp():.0f}{ext}"
        ...
        doc_in = Document(
            ...
            document_metadata={"original_filename": original_filename or safe_filename, "size": len(content)},
        )
        return await self.case_repository.create_document(doc_in=doc_in)
    ...
    @tool_registry.register(
        ...
        parameters=[
            ToolParameter(
                "case_id", "string", "The UUID of the case for which to schedule a reminder"
            ),
            ToolParameter(
                "reminder_date", "string", "The date/time for the reminder in ISO format"
            ),
    ...
    async def schedule_reminder(
        self, case_id: UUID, reminder_date: str, note: str
    ) -> Dict[str, Any]:
    ...
    @tool_registry.register(
        ...
        parameters=[
            ToolParameter(
                ...
                "string",
                "Type of legal event",
                enum=["payment", "appeal", "complaint", "response_to_claim", "cassation"],
            ),
    ...
    async def compute_deadline(self, event_type: str, event_date: str) -> Dict[str, Any]:
        ...
        if not deadline_info:
            return {
                "error": f"Unknown event type: {event_type}. Valid types: {list(deadlines.keys())}"
            }
...
def get_case_management_service(
    request: Request, case_repository: CaseRepositoryDep
) -> CaseManagementService:
    return request.app.state.manager.inject_service(CaseManagementService).with_case_repository(
        case_repository
    )
