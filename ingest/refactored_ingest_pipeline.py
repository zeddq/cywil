...
class RefactoredIngestOrchestrator:
    ...
    async def validate_ingestion(self) -> Dict[str, Any]:
        ...
        statute_status = await self.statute_service.get_ingestion_status() if self.statute_service else {"status": "not_initialized"}  # type: ignore[attr-defined]
        court_status = await self.court_service.get_sn_processing_status() if self.court_service else {"status": "not_initialized"}  # type: ignore[attr-defined]
        embedding_stats = await self.embedding_service.get_embedding_statistics() if self.embedding_service else {"status": "not_initialized"}  # type: ignore[attr-defined]
